from oem_updater.core.helpers import median
...
class Elapsed(object):
    ...
    @classmethod
    def calculate_statistics(cls, raw_samples):
        if not raw_samples:
            return None
    ...
    @classmethod
    def format_statistics(cls, line_format='%-32s %-12s %-12s %-10s %-10s %-10s %-10s %-10s'):
        # Display header
        yield line_format % ('Function', 'Result', 'Samples', 'Minimum', 'Maximum', 'Average', 'Median', 'Total')
        ...
        for f_name, groups in cls.samples.items():
            for result, samples in groups.items():
                statistics = cls.calculate_statistics(samples)

                if not statistics:
                    yield '%-32s %s' % (f_name, result)
                    continue
